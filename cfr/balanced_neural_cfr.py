"""
Balanced Neural-Enhanced CFR for 2-Player Texas Hold'em
A less conservative version with more aggressive play while maintaining hand strength awareness
"""
...
class HandEvaluator:
    ...
    def _init_preflop_rankings(self):
        """Initialize preflop hand rankings - LOOSER for balanced play"""
        # More aggressive preflop ranges
        rankings = {}
        premium = ['AA', 'KK', 'QQ', 'AKs', 'JJ', 'AKo']
        strong = ['TT', '99', 'AQs', 'AJs', 'KQs', 'AQo', '88', 'ATs']
        playable = ['77', '66', 'KJs', 'QJs', 'JTs', 'AJo', 'KQo', 'A9s', 'KTs', 'QTs']
        speculative = ['55', '44', '33', '22', 'T9s', '98s', '87s', '76s', 'A8s', 'A7s', 'A6s', 'A5s', 'A4s', 'A3s', 'A2s']
        ...
        for hand in speculative:
            rankings[hand] = 0.55  # Play more speculative hands
...
class BalancedNeuralCFR:
    """Balanced Neural-Enhanced CFR - Less conservative, more aggressive play"""
    ...
    def train(self, verbose: bool = True):
        ...
        if verbose:
            print("Starting Balanced Neural-Enhanced CFR Training")
    ...
    def _adjust_strategy_for_strength(self, base_strategy: np.ndarray,
        ...
        if category == HandStrengthCategory.MONSTER:
            ...
            # Weak hands - MORE AGGRESSIVE BLUFFING in balanced version
            adjusted[ImprovedAction.FOLD.value] *= 1.0  # Less folding
            adjusted[ImprovedAction.CHECK.value] *= 1.1  # Slightly less checking
            adjusted[ImprovedAction.CALL.value] *= 0.9
            # INCREASED bluff frequency for balanced play
            adjusted[ImprovedAction.BET_SMALL.value] *= 0.8
            adjusted[ImprovedAction.BET_MEDIUM.value] *= 0.7
            adjusted[ImprovedAction.BET_LARGE.value] *= 0.8  # More bluffs

        # Drawing hands - MUCH MORE AGGRESSIVE in balanced version
        if strength_info['is_drawing']:
            # Semi-bluff aggressively with draws
            adjusted[ImprovedAction.BET_SMALL.value] *= 1.5  # More semi-bluffs
            adjusted[ImprovedAction.BET_MEDIUM.value] *= 1.4
            adjusted[ImprovedAction.BET_LARGE.value] *= 1.2  # Occasional big semi-bluffs
            adjusted[ImprovedAction.CHECK.value] *= 0.7  # Less passive with draws
    ...
    def _print_final_stats(self):
        ...
        print("\n" + "=" * 60)
        print("BALANCED NEURAL-ENHANCED CFR TRAINING COMPLETE")
        print("=" * 60)
    ...
    def _get_default_strategy(self, strength_info: Dict, pot_size: int, to_call: int) -> np.ndarray:
        ...
        if category in [HandStrengthCategory.MONSTER, HandStrengthCategory.VERY_STRONG]:
            ...
            # Medium hands - MORE AGGRESSIVE in balanced version
            strategy[ImprovedAction.FOLD.value] = 0.08  # Much less folding
            strategy[ImprovedAction.CHECK.value] = 0.30  # Still check sometimes
            strategy[ImprovedAction.CALL.value] = 0.27  # Call more
            strategy[ImprovedAction.BET_SMALL.value] = 0.20  # More small bets
            strategy[ImprovedAction.BET_MEDIUM.value] = 0.12  # More medium bets
            strategy[ImprovedAction.BET_LARGE.value] = 0.025  # Occasional large
            strategy[ImprovedAction.BET_OVERBET.value] = 0.004
            strategy[ImprovedAction.ALL_IN.value] = 0.001

        else:
            # Weak hands - AGGRESSIVE BLUFFING in balanced version
            strategy[ImprovedAction.FOLD.value] = 0.20  # Much less folding (was 0.35)
            strategy[ImprovedAction.CHECK.value] = 0.30  # Less passive checking
            strategy[ImprovedAction.CALL.value] = 0.20  # More floating
            strategy[ImprovedAction.BET_SMALL.value] = 0.15  # More bluffs (was 0.08)
            strategy[ImprovedAction.BET_MEDIUM.value] = 0.10  # More bluffs (was 0.04)
            strategy[ImprovedAction.BET_LARGE.value] = 0.04  # Big bluffs (was 0.02)
            strategy[ImprovedAction.BET_OVERBET.value] = 0.008
            strategy[ImprovedAction.ALL_IN.value] = 0.002
    ...
    def save(self, filename: str):
        ...
        print(f"Balanced Neural-Enhanced CFR saved to {filename}")
    ...
    def load(self, filename: str):
        ...
        print(f"Balanced Neural-Enhanced CFR loaded from {filename}")
        print(f"  Information sets: {len(self.nodes):,}")
        print(f"  Iterations: {self.iteration_count}")
